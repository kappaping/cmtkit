...
eegsss=frg.gridenergies(H0,ltype,rs,NB,RDV,Nrfl,nf,prds,ks,Nkc,kqids,Eee,W,mu,Ngmax)
T=0.02
CHI=frg.baresuscepmat(Eee,kqids,eegsss,T,chipm)
data=[np.sum(CHI[nq]) for nq in range(Nqi)]
print('T =',T,', max =',np.max(data),', data[',qis[nq],'] =',data[nq])
...
plbd.plotbz(ltype,prds,qis,todata=todata,data=data,ptype='gd',dks=dqs,bzop=bzop,tolabel=tolabel,tosave=tosave,filetfig=filetfig)
