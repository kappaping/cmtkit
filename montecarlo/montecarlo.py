...
def mcflip(fls,Js,T,rs,Nltc,Nr,bc,mtype,fltype):
    for nf in range(Nr):
        r=random.choice(rs)
        flr=fls[ltc.rid(r,Nltc)]
        bc=1
        pairsr=ltc.pairs(r,Nltc[0],bc,mtype)
...
def equilibrate(fls,Js,T,rs,Nltc,Nr,bc,mtype,fltype,NEQ):
    # Approach equilibrium
    [mcflip(fls,Js,T,rs,Nltc,Nr,bc,mtype,fltype) for n in range(NEQ)]
    

def sampling(fls,Js,T,rs,Nltc,Nr,bc,mtype,fltype,NMC):
    '''
    ...
    esqt=0.
    for n in range(NMC):
        mcflip(fls,Js,T,rs,Nltc,Nr,bc,mtype,fltype)
        mn=magnetization(fls,rs,Nltc,Nr)
        mt+=mn
        msqt+=np.linalg.norm(mn)**2
        en=energy(fls,Js,rs,Nltc,Nr,mtype)
        et+=en
        esqt+=en**2
...
def energy(fls,Js,rs,Nltc,Nr,mtype):
    e0=0.
    for r in rs:
        bc=1
        pairsr=ltc.pairs(r,Nltc[0],bc,mtype)
